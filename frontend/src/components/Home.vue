  ...
  <div class="px-4 py-6 sm:px-0">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
      <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        <h3 class="text-xl font-semibold text-white mb-2">Visual Automation</h3>
      ...
      <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        <h3 class="text-xl font-semibold text-white mb-2">Task Builder</h3>
        <p class="text-gray-300">
      ...
      <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        <h3 class="text-xl font-semibold text-white mb-2">File Integration</h3>
        <p class="text-gray-300">
    ...
    <div class="text-center">
      ...
      <!-- Test Results -->
      <div v-if="testResult" class="mt-4 p-4 rounded-lg" :class="testResult.status === 'success' ? 'bg-green-800' : 'bg-red-800'">
        <p class="text-white">{{ testResult.message }}</p>
      </div>
    </div>
    ...
    <div class="mt-12 bg-gray-800 rounded-lg p-6 border border-gray-700">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
          <div class="text-2xl font-bold text-green-400">{{ systemStatus.backend ? '✓' : '✗' }}</div>
          <div class="text-sm text-gray-300">Backend</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-green-400">{{ systemStatus.database ? '✓' : '✗' }}</div>
          <div class="text-sm text-gray-300">Database</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-green-400">{{ systemStatus.vnc ? '✓' : '✗' }}</div>
          <div class="text-sm text-gray-300">VNC Server</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-blue-400">{{ systemStatus.timezone }}</div>
          <div class="text-sm text-gray-300">Timezone</div>
        </div>
      </div>
    </div>
  </div>
...
const checkSystemStatus = async () => {
  try {
    ...
    systemStatus.value = {
      backend: true,
      database: true, // Assume true if backend is responding
      vnc: true, // Assume true if backend is responding
...
const testBrowser = async () => {
  ...
  try {
    ...
    testResult.value = {
      status: 'error',
      message: 'Failed to test browser: ' + error.message
    }
  } finally {
