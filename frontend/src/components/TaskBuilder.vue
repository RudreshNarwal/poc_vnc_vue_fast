    ...
    <form @submit.prevent="saveTask" class="space-y-8">
        ...
        <div class="grid grid-cols-1 gap-6">
          <div>
            ...
            <input
              ...
              v-model="task.name"
              required
              class="mt-1 block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
      ...
      <div class="bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-medium text-gray-900">Automation Steps</h2>
          <button
            type="button"
            @click="addStep"
        ...
        <div v-if="task.steps.length === 0" class="text-center py-8 text-gray-500">
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select v-model="step.action" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
              ...
              <div v-if="needsTarget(step.action)">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ getTargetLabel(step.action) }}</label>
                <input type="text" v-model="step.target" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" :placeholder="getTargetPlaceholder(step.action)" />
              </div>

              <div v-if="needsValue(step.action)">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ getValueLabel(step.action) }}</label>
                <input type="text" v-model="step.value" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" :placeholder="getValuePlaceholder(step.action)" />
              </div>
            </div>

            <div class="mt-3">
              <label class="block text-sm font-medium text-gray-700 mb-1">Description (Optional)</label>
              <input type="text" v-model="step.description" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="Describe this step" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
        ...
        <div v-if="task.prerequisites.length === 0" class="text-center py-4 text-gray-500">No prerequisites required.</div>

        <div v-else class="space-y-3">
          <div v-for="(prereq, index) in task.prerequisites" :key="index" class="bg-gray-50 rounded-lg p-3 border border-gray-200">
            <div class="flex justify-between items-start mb-2">
              <span class="text-sm font-medium text-gray-700">Prerequisite {{ index + 1 }}</span>
              <button type="button" @click="removePrerequisite(index)" class="text-red-600 hover:text-red-700">Remove</button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <select v-model="prereq.type" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
              ...
              <div>
                <input type="text" v-model="prereq.name" placeholder="Name" class="block w-full rounded-md bg-white border-gray-300 text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex justify-end space-x-3">
        <router-link to="/tasks" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Cancel</router-link>
